...
export class CanvasManager {
    ...
    try {
      ...
      for (const canvas of this._canvases.values()) {
        ...
        if (idx !== -1) {
          ...
          this._emit('placementRemoved', { canvasId: canvas.id, placement: removed });
          break;
        }
      }
    } catch (error) {
  ...
  _handleCanvasUpdated(message) {
    ...
    if (canvas) {
      ...
      this._emit('canvasUpdated', { canvas, updates: message.updates, source: 'broadcast' });
    }
  }
  ...
  _handleCanvasDeleted(message) {
    ...
    this._emit('canvasDeleted', { canvasId: message.canvasId, canvas, source: 'broadcast' });
  }
  ...
  _handlePlacementAdded(message) {
    ...
    if (canvas) {
      ...
      canvas.placements.push(placement);
      this._emit('placementAdded', { canvasId: message.canvasId, placement, source: 'broadcast' });
    }
  }
  ...
  _handlePlacementUpdated(message) {
    const canvas = this._canvases.get(message.canvasId);
    if (canvas) {
      const idx = canvas.placements.findIndex((p) => p.id === message.placement.id);
      if (idx !== -1) {
  ...
  _handlePlacementRemoved(message) {
    const canvas = this._canvases.get(message.canvasId);
    if (canvas) {
      const idx = canvas.placements.findIndex((p) => p.id === message.placementId);
      if (idx !== -1) {
        const removed = canvas.placements.splice(idx, 1)[0];
