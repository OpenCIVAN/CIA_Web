...
export class SubsetManager {
  ...
  async createSubset(canvasId, options = {}) {
    ...
    const payload = {
      ...
      placementIds: options.placementIds || Array.from(this._selectedPlacementIds),
    };
  ...
  enterFocusMode(subsetId, currentViewport) {
    const subset = this._subsets.get(subsetId);
    if (!subset) {
      console.error('SubsetManager: Cannot enter focus mode - subset not found');
      return;
    }
    ...
    console.log(`SubsetManager: Entering focus mode with subset "${subset.name}"`);
  ...
  _handleSubsetDeleted(message) {
    ...
    this._emit('subsetDeleted', { subsetId: message.subsetId, subset, source: 'broadcast' });
  }
